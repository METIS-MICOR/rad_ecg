...
#CLASS EDA
class EDA(object):
	def __init__(self, ecg_data, wave):

		self.dataset = datasets.fetch_openml(
			data_id=43008,  
			return_X_y=False,
			as_frame=True,
		)

		self.task = "classification"									#!MUSTCHANGEME
		self.target = self.dataset["data"]["DEATH_EVENT"]
		# self.target = self.dataset["target"]
		self.data = self.dataset["data"].drop("DEATH_EVENT", axis=1) 
		# self.data = self.dataset["data"]
		self.data_description = self.dataset['DESCR']
		#Sometimes these aren't always included in sklearn's datasets
		if 'target_names' in self.dataset.keys():
							
			if isinstance(self.dataset["target_names"], list):
				# self.target_names = sorted(Counter(self.target).keys())
				self.target_names = np.array(["living", "deceased"], dtype="U9")
			else:
				self.target_names = self.dataset['target_names']

		if 'feature_names' in self.dataset.keys():
			self.feature_names = list(self.dataset['feature_names'])
...
class FeatureEngineering(EDA):
	...
	def engineer(self, features:list, transform:bool, display:bool, trans:str):
		...
		"""
		#TODO - Need to implement a group input here. 
		...
		#Quick target formatting to include groupings in the histograms
