...
import sys
import os
from rad_ecg import utils
from rad_ecg import support
...
#Make a directory underneath the data/json folder with current date
# os.makedirs(f'./data/json/{current_date}', exist_ok=True)

#formats for the logging file and terminal respectively
...
# FUNCTION Short time fourier transform
def STFT(new_peaks_arr:np.array,
			peak_info:np.array, 
			rolled_med:np.array, 
			st_fn:tuple, 
			plot_fft:bool=False):
...
#FUNCTION R Peak Search
@log_time
def R_peak_search(ecg_data:dict, 
					plot_fft:bool=False,
					plot_errors:bool=False):
	...
	Returns:
		...
		#wave_sections (np.array): Sections of the wave.  Last column is boolean of valid or not
	"""
	...
	while len(peak_que) > 0:
		...
		if not found_wave:
			...
			if R_peaks.size < 4 or R_peaks.size > 60:
				...
				sect_valid, new_peaks_arr = utils.STFT(new_peaks_arr, peak_info, rolled_med, (section_counter, start_p, end_p), plot_fft)
...
#NOTE Load clinical data
dat_files, mib_files, head_files = utils.get_records('cam_data')

#NOTE Load signal data 
record = wfdb.rdrecord(head_files[0].strip('.hea'),
					sampfrom=0,
					sampto=None,
					channels=[0])
...
wave_sections = utils.segment_ECG(wave, fs)[500:6000]
...
#BUG - Might need to make this global for other utils scripts
ecg_data = {
...
ecg_data = R_peak_search( 
	ecg_data, 
	plot_fft=False, 
	plot_errors=True, 
)

log_path = f'./data/logs/{current_date}_.log'
...
#Cols of ecg_data['peaks']
	# 0: R Peak index					- int32
	# 1: Peak validity					- int32
	
#Cols of ecg_data['rolling_med']
	# 0: rolling_med					- float32

#Cols of ecg_data['Section_info']
	...
	# 5 : Avg HR						- float32
	# 6 : SDNN	 						- float32
	# 7 : Min HR Diff					- float32
	# 8 : Max HR Diff					- float32
	# 9 : RMSSD							- float32 
	# 10: NN50							- float32
	# 11: PNN50							- float32

#Cols of ecg_data['interior_peaks']
	# 0 : P peak		  (idx) - int32
	# 1 : Q peak		  (idx) - int32
	# 2 : R peak 		  (idx) - int32
	# 3 : S peak		  (idx) - int32
