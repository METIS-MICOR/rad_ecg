...
def load_graph_objects(run:str):
	...
	def update_plot(val):
		...
		if inners.size > 0:
			...
			#TODO Get chart labeling to work with ecg_data['peaks'] info for R peak labeling. 
			#TODO And fix below for labeling onsets. 

			# [add_cht_labels(inners[:, key], wave[inners[:, key].flatten()], ax_ecg, val[0]) for key, val in peak_dict_ext.items()]
	...
	ecg_data = {
		...
		# 'rolling_med': np.genfromtxt(fpath+"rolling_med.csv", delimiter=',', dtype=np.float32),
	...
	#TODO - Have that sections logs loaded up to when you
...
def main():
	run = "01-07-2024_09-51-07"  	#CAM zKGYG-WNAM2
...
# run = "01-07-2024_07-28-19"  	#CAM JF73U-8S2G5
# 								# rolls |  seps   | heights |  FFT  | Total
# 								# 174   |  180    |  173    |   2   |  529
...
#IDEA
	#Upgrades for chart.  
	#Need a better table summary.  Perhaps a split window of 90% ECG
	#then the table moved over a bit so its not shove into the side.  

#Old scratch code for examining things

# res = Counter(ecg_data['section_info']['fail_reason'])
# rolls = sum([val for key, val in res.items() if "roll" in key])
# seps = sum([val for key, val in res.items() if "sep" in key])
# heights = sum([val for key, val in res.items() if "height" in key])
# FFTs = sum([val for key, val in res.items() if "FFT" in key])
# print("rolls", " seps  ", "heights", " FFT ", "Total", sep= " | " )
# print(rolls, seps, heights, FFTs, sum([rolls, seps, heights, FFTs]), sep = "    |    ")

# invalid_perc = np.where(ecg_data['section_info']['valid']==0)[0]
# perc = invalid_perc.shape[0] / 4000
# print(f'{perc *100}')
# HR_ids = np.nonzero(ecg_data["section_info"]["Avg_HR"])
# print(np.mean(ecg_data['section_info'][HR_ids]["Avg_HR"]))

# R_ids = np.nonzero(ecg_data["section_info"]["RMSSD"])
# print(np.mean(ecg_data['section_info'][R_ids]["RMSSD"]))


#Quick Code for checking find_peaks functionality through RMSSD
#Lookign like 98% will work a little better.  Keep the height requirement
#at 95 and rerun small sects.  
#TODO uncomment 2 lines below
# RMSSD = ecg_data['section_info'][['wave_section', 'RMSSD']]
# print(sorted(RMSSD, key= lambda x:x[1], reverse=True)[:20])

# start_p, end_p = ecg_data['section_info'][sect_slider.val][['start_point', 'end_point']]
# wave_chunk = wave[start_p:end_p]
# R_peaks, peak_info = ss.find_peaks(
# 	wave_chunk.flatten(), 
# 	prominence = np.percentile(wave_chunk, 98), #99 -> stock
# 	height = np.percentile(wave_chunk, 92),     #95 -> stock
# 	distance = round(fs*(0.200)) #Can't have a heart rate faster than 200ms
# )

# R_peaks = R_peaks + start_p
# R_peaks
# np.diff(R_peaks)

# #checking logs for lookback times. 
# lookbacks = [x for x in log if "QRS lookback was" in x]
# times = [x.split(" ")[3] for x in lookbacks]
# times_h = [float(x[:-1]) for x in times if "h" in x]
# times_m = [float(x[:-1]) for x in times if "m" in x]
# print(sorted(times_m, key=lambda x:x, reverse=True)[:20])



#Checking something.  Not sure what. 
# test_arr = {
# 	"section_info":ecg_data['section_info'][:1200],
# 	"peaks":ecg_data['peaks'][:1200],
# 	"interior_peaks":ecg_data['interior_peaks'][:1200]
# }

# ids = np.nonzero(test_arr['section_info']['valid'])[0]
# np.median(wave[ids])

# # #Checking sizes of arrays
# for x in ecg_data.keys():
# 	print(f'{x}\t\t{sys.getsizeof(ecg_data[x])*.000_001:.2f} MB')

# # # 	######################################################################################################

# # # # For looking at nulls
# print('Peak | Missing | full counts | Perc. of total')
# PNull = len(ecg_data['interior_peaks'][np.where(ecg_data['interior_peaks'][:, 0]==0)[0], :6])
# QNull = len(ecg_data['interior_peaks'][np.where(ecg_data['interior_peaks'][:, 1]==0)[0], :6])
# RNull = len(ecg_data['interior_peaks'][np.where(ecg_data['interior_peaks'][:, 2]==0)[0], :6])
# SNull = len(ecg_data['interior_peaks'][np.where(ecg_data['interior_peaks'][:, 3]==0)[0], :6])
# TNull = len(ecg_data['interior_peaks'][np.where(ecg_data['interior_peaks'][:, 4]==0)[0], :6])
# print(f"P \t{PNull}\t{ecg_data['interior_peaks'][:, 0].size}\t\t{(PNull/ecg_data['interior_peaks'][:, 0].size)*100:.2f}")
# print(f"Q \t{QNull}\t{ecg_data['interior_peaks'][:, 1].size}\t\t{(QNull/ecg_data['interior_peaks'][:, 1].size)*100:.2f}")
# print(f"R \t{RNull}\t{ecg_data['interior_peaks'][:, 2].size}\t\t{(RNull/ecg_data['interior_peaks'][:, 2].size)*100:.2f}")
# print(f"S \t{SNull}\t{ecg_data['interior_peaks'][:, 3].size}\t\t{(SNull/ecg_data['interior_peaks'][:, 3].size)*100:.2f}")
# print(f"T \t{TNull}\t{ecg_data['interior_peaks'][:, 4].size}\t\t{(TNull/ecg_data['interior_peaks'][:, 4].size)*100:.2f}")

# #? Checking histogram of returned segment vals.
# test = {
# 	7:'PR',
# 	8:'QRS',
# 	10:'QT'
...
# 	ids = np.nonzero(ecg_data['interior_peaks'][:, key])
# 	hist, bins = np.histogram(ecg_data['interior_peaks'][ids[0], key], bins=10)
# 	plt.figure(figsize=(8, 6))
# 	plt.hist(ecg_data['interior_peaks'][ids[0], key], bins)
# 	plt.title(f'Histogram of {val} Seg', fontsize=18)
# 	plt.xlabel("Bins")
# 	plt.ylabel("Counts")
# 	plt.annotate(f'Avg {val} Seg {np.mean(ecg_data["interior_peaks"][ids[0], key]):.0f} ms', xy=(0.70, 0.95), xycoords='axes fraction', fontsize=12)
# 	plt.annotate(f'Max {val} Seg {np.max(ecg_data["interior_peaks"][ids[0], key]):.0f} ms', xy=(0.70, 0.91), xycoords='axes fraction', fontsize=12)
# 	plt.annotate(f'Min {val} Seg {np.min(ecg_data["interior_peaks"][ids[0], key]):.0f} ms', xy=(0.70, 0.87), xycoords='axes fraction', fontsize=12)
